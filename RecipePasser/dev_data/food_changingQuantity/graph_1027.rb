	...
	file.each_line do |recipeLine|
		...
		dataArray.each do |dataLine|
			...
			if arr[0].to_s == recipeArray[0].to_s && arr[2].to_s != 0.00
				score = 0
				str = arr[1]
				score = (1.0 - (arr[2].to_f - recipeArray[3].to_f)/recipeArray[3].to_f).abs
				# p(dataArray.find_index(dataLine))
				dataArray.delete_at(dataArray.find_index(dataLine))
				if score < 0.7 || score > 1.3
					break
				else
					if lowerArray[0][1].to_f > score
						lowerArray[0][0] = str
						lowerArray[0][1] = sprintf("%.5f",score)
						lowerArray = lowerArray.sort {|a,b|
							a[1].to_f <=> b[1].to_f
						}
						lowerArray.reverse!
					elsif upperArray[0][1].to_f < score
						upperArray[0][0] = str
						upperArray[0][1] = sprintf("%.5f",score)
        			...
        			# p(count)
        			# p(score)
        			outputArray.push(recipeArray[0])
        			p(recipeArray)
        			sum = sum + score
        			count = count + 1
        			break
...
count = 0
File.open("correct_data.txt", "w") do |f|
	outputArray.each do |word|
		f.write(word.to_s + "\n")
	end
end
